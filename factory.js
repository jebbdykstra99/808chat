(function () {
  ...
  function renderTrends(quiet) {
    ...
    liveFetch().then(function (cards) {
      var extra = outboundCards();
      var merged = railKind() === 'nws-forecast' ? (extra || []).concat(cards || []) : (cards || []).concat(extra);
  ...
  function storiesComposePlaceholder() {
    return (site && site.composePlaceholder) || "What's happening in the 808?";
  }
        ...
        setDemo: function (list, ownUid) {
          ...
          liveStories = (list || []).map(function (s, i) {
            return {
              ...
              name: s.name || (site && site.name) || '808',
              handle: s.handle || SITE_ID || '808',
              type: s.type || 'text',
              text: s.text || '',
              mediaUrl: s.mediaUrl || '',
